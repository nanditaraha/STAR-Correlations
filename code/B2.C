...
//--------------------------------------------------------------------------
//
//	to plot balance functions		WJL 12/21
//		
//	second version
...
void B2(){
  ...
  //--------------------------------------------
  TString* onefile		= new TString("root_files/reader_m40_data_24_cuts1_qcut_xp.root");
  //DATA
  //TString* onefile		= new TString("root_files/reader_c4_urqmdLN_16_cuts1.root");
  //SIMULATION!!
  //TString* onefile		= new TString("/tier2/home/groups/rhi/share/readerroot2021_22/reader_c4_urqmdLN_16_cuts1.root");
  ...
  TString base		= TString("B2_mult") + ofstring;
  TString outfile		= dir + base + TString(".pdf");
  TString outfileO	= dir + base + TString(".pdf(");
  TString outfileC	= dir + base + TString(".pdf]");
  ...
  for (int ipart=0;ipart<3;ipart++){		// pi, then K, then p
	...
	*/
	...
	continue; 
      ...
      /*if(icent==16){
      cout<<"Corr. Int. calculations: icent="<<icent<<" ipart="<<ipart<<" \t "
	  <<"prefp="<<prefp<<"\t prefm="<<prefm<<"\t C2pp="<<intC2pp<<"\t C2mm="<<intC2mm
	  <<"\t C2pm="<<intC2pm<<"\t C2mp="<<intC2mp<<"\t SR +ve "<<sumrule_B2p
	  <<"\t SR -ve "<<sumrule_B2m<<endl;
	  }*/
  ...
  for (int icent=16;icent<=16;icent++){	//!!!!!!!!!!!! PAINT CENTRAL ONLY FOR NOW......
	...
	hR2cdphi[icent][kpaty]->Draw("hist same");
      ...
      ++itxt; txt[itxt]->DrawLatex(0.7,0.88,Form("INT: %.2f",hB2cpdy[icent][ipart]->Integral()/hB2cpdy[icent][ipart]->GetNbinsX()));
      ...
      ++itxt; txt[itxt]->DrawLatex(0.7,0.88,Form("INT: %.2f",hB2cmdy[icent][ipart]->Integral()/hB2cmdy[icent][ipart]->GetNbinsX()));
      ...
      ++itxt; txt[itxt]->DrawLatex(0.7,0.88,Form("INT: %.2f",hB2cdy[icent][ipart]->Integral()/hB2cdy[icent][ipart]->GetNbinsX()));
      ...
      ++itxt; txt[itxt]->DrawLatex(0.7,0.88,Form("INT: %.2f",hB2cpdphi[icent][ipart]->Integral()/hB2cpdphi[icent][ipart]->GetNbinsX()));
      ...
      ++itxt; txt[itxt]->DrawLatex(0.7,0.88,Form("INT: %.2f",hB2cmdphi[icent][ipart]->Integral()/hB2cmdphi[icent][ipart]->GetNbinsX()));
      ...
      ++itxt; txt[itxt]->DrawLatex(0.7,0.88,Form("INT: %.2f",hB2cdphi[icent][ipart]->Integral()/hB2cdphi[icent][ipart]->GetNbinsX()));
